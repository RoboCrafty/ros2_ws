...
int main(int argc, char** argv)
{
    ...
    //实例化一个对象
    rclcpp::spin(std::make_shared<TarkbotRobot>());

    rclcpp::shutdown();
    return 0;
} 
// int akm_servo_offset_ = 120;
/*
 * @功  能  构造函数，机器人初始化
 */
TarkbotRobot::TarkbotRobot(
    const std::string & node_name_ = "tarkbot_robot_node",
    const std::string & namespace_ = "",
    const rclcpp::NodeOptions & node_options_ = (
        rclcpp::NodeOptions().automatically_declare_parameters_from_overrides(true)
    )
):rclcpp::Node(node_name_, namespace_, node_options_)
{
    ...
    // this->declare_parameter<std::string>("robot_port", "/dev/ttyTHS1");
    // this->declare_parameter("akm_servo_offset_");
    ...
    this->get_parameter<std::string>("battery_topic", bat_topic_);
    this->get_parameter<std::string>("cmd_vel_topic", cmd_vel_topic_);

    //参数初始化
    this->get_parameter<std::string>("robot_port", serial_port_);
    this->get_parameter<bool>("pub_odom_tf", pub_odom_tf_);
    this->get_parameter<int>("akm_servo_offset", akm_servo_offset_);

    //实例化发布者对象
    odom_pub_ = this->create_publisher<nav_msgs::msg::Odometry>(odom_topic_, rclcpp::QoS(50));
    imu_pub_  = this->create_publisher<sensor_msgs::msg::Imu>(imu_topic_,rclcpp::QoS(50));    
    bat_pub_  = this->create_publisher<std_msgs::msg::Float32>(bat_topic_,rclcpp::QoS(10));
    transform_broadcaster_ = std::make_unique<tf2_ros::TransformBroadcaster>(this);  //根本目的在于将 tf TransformBroadcaster 类转换为 tf2 Broadcaster http://wiki.ros.org/tf2/Tutorials/Migration/TransformBroadcaster 类
    //依赖于tf2_ros包，可能还有tf2_geometry_msgs

    cmd_vel_sub_         = this->create_subscription<geometry_msgs::msg::Twist>("cmd_vel",rclcpp::QoS(1),std::bind(&TarkbotRobot::cmdVelCallback,this,std::placeholders::_1));
    ...
    RCLCPP_INFO(this->get_logger(),"Initialize Robot Base : \033[32m Done! \033[0m...");

}

/*
 * @功  能  析构函数，当对象结束其生命周期时系统会调用这个函数
 */
TarkbotRobot::~TarkbotRobot()
{ 
    ...
    //提示信息
    RCLCPP_INFO(this->get_logger(),"Shutting down : \033[32m Done! \033[0m...");
}
...
bool TarkbotRobot::openSerialPort()
{
    ...
    if(err_code_)
    {
        printf("Open Port: %s Failed! Aboart!",serial_port_.c_str());
        return false;
    }

    //初始化串口参数
    serial_ptr_->set_option(boost::asio::serial_port_base::baud_rate(230400));
