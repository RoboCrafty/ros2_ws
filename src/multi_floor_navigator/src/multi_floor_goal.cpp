...
public:
    CustomNode() : Node("multi_floor_goal")
    {
        ...
        feedback_sub_ = this->create_subscription<nav2_msgs::action::NavigateToPose::Feedback>(
            "navigate_to_pose/feedback", 10, std::bind(&CustomNode::feedbackCallback, this, std::placeholders::_1));
    }
...
private:
    void goal_callback(const geometry_msgs::msg::Pose::SharedPtr msg)
    {
        ...
        if (msg->position.z != current_floor_)
        {
            ...
            auto elevator_goal = std::make_shared<geometry_msgs::msg::PoseStamped>();
            ...
            RCLCPP_INFO(this->get_logger(), "Calling elevator to current floor: %d", starting_floor_);
        }
        else
        {   
            msg->position.z = 0.0;
            auto goal_on_same_floor = std::make_shared<geometry_msgs::msg::PoseStamped>();
    ...
    rclcpp::Subscription<nav2_msgs::action::NavigateToPose::Feedback>::SharedPtr feedback_sub_;
